#include "FilterMidiChannelMsg.h"


namespace MidiRoute {
void filterMidiChannelMsg(RtMidiWrap::MidiEvent &in, RangeMap &fromChannel,
                          RangeMap &fromCommand, RangeMap &fromData1,
                          RangeMap &fromData2,
    ...
    if (in.msgtype() == RtMidiWrap::MIDI_MSG_TYPE::MIDI_CHANNEL_MESSAGES) {
        ...
        if (fromChannel.isInRange(old_channel)) {
            new_channel = fromChannel.getVal((float)old_channel);
        }
        ...
        if (fromData1.isInRange(old_data1)) {
            new_data1 = fromData1.getVal((float)old_data1);
        }
        ...
        passedFromFilter = passedFromFilter && new_data2 >= 0 && new_data1 >= 0 &&
                           new_channel >= 0 && new_command >= 0;

        if (deleteIfNot == MIDI_FILTER_ACTION_IF_NOT::DELETE_IF_NOT &&
            !passedFromFilter) {
            in.eventStatus = RtMidiWrap::EVENT_STATUS::DELETED;
        } else if (deleteIfNot == MIDI_FILTER_ACTION_IF_NOT::DELETE_IF_IS &&
                   passedFromFilter) {
            in.eventStatus = RtMidiWrap::EVENT_STATUS::DELETED;
        } else {
            if (passedFromFilter) {
                ...
                if (new_command == RtMidiWrap::CommonStatic::MIDI_CHANNEL_MESSAGES::
                        channelaftertouch ||
                    new_command == RtMidiWrap::CommonStatic::MIDI_CHANNEL_MESSAGES::
                                                                                                             programchange) {
                    if (3 == sndVector.size()) {
                ...
                if (new_command ==
                    RtMidiWrap::CommonStatic::MIDI_CHANNEL_MESSAGES::pitchbend) {
